...
namespace TeamProject
{
    public class XLDL
    {
        SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=D:\ProjectAsp\ProjectAsp\TeamProject\App_Data\WebPhuKien.mdf;Integrated Security=True");
        public void open()
        {
        ...
        public string docDuyNhatStored(string query, string[] values, string[] paras)
        {
            open();
            SqlCommand command = new SqlCommand(query, connect);
            command.CommandType = CommandType.StoredProcedure;
